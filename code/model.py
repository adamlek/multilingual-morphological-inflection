...
class Inflector(nn.Module):
    def __init__(self, start_idx, end_idx, pad_idx, vocab, tags, langs, ops):
        ...
        self.pos2emb = nn.Embedding(512, args.embedding_dim)
        ...
        # make this a function of the loss? sc_e/(1/loss)?
        self.sc_decay_rate = self.sc_e/args.prob_decay_steps 
        
        # discriminators
        #self.lang_discriminator = nn.Linear(args.hidden_dim*2, langs, bias=False)
        self.lang_discriminator = nn.Linear(args.hidden_dim*6, langs, bias=False)
        ...
        # dropouts
        self.encoder_drp = nn.Dropout(0.3)
        self.decoder_drp = nn.Dropout(0.2)
        self.lang_drp = nn.Dropout(0.2)
    ...
    def forward(self, lemma, lemma_mask, tags, tags_mask, langs=None, gold=None, tf=True, test=False):
        
        if test:
            #print('Running inference with beam-decode')
            return self.inference(lemma, lemma_mask, tags, tags_mask)
        
        # encode input
        lemma_h, tags_h, l_ht, l_ct = self.encode_inputs(lemma, lemma_mask, tags, tags_mask)
        
        ht, ct = self.get_zeros_init(lemma_h.size(0), args.hidden_dim)
        
        # set first decoder input to <SOS>
        start_t = self.char2emb(torch.tensor([self.start_idx]*lemma_h.size(0), device=device).long())
        ...
        # decode sequence
        decode_hiddens, final_hcx, decode_pred = self.batch_decode(start_t, 
                                                                ht, ct, 
                                                                lemma_h, lemma_mask, 
                                                                tags_h, tags_mask, 
                                                                gold[:,1:],
        ...
        #decode_pred = self.char_classifier(decode_out)
        opsy_pred = self.ops_discriminator(decode_hiddens)

        l_ht = torch.cat([l_ht[0,:,:], l_ht[1,:,:]], -1)
        l_ct = torch.cat([l_ct[0,:,:], l_ct[1,:,:]], -1)
        ## encoder hidden states + final state of decoder
        hcx_lemma_decoder = torch.cat([l_ht, l_ct, final_hcx], -1)
        
        lang_pred = self.language_identification(hcx_lemma_decoder)
        
        return decode_pred, lang_pred, opsy_pred, opsx_pred
    ...
    def encode_inputs(self, lemma, lemma_mask, tags, tags_mask):
        ...
        """
        lemma_e = self.encoder_drp(self.char2emb(lemma))
        tags_e = self.encoder_drp(self.tag2emb(tags))
        
        lemma_h, (ht, ct) = self.lemma_encoder(lemma_e)
        # TODO: lemma_h = lemma_mask * lemma_h # with masked_fill
    ...
    def batch_decode(self, x, hx, cx, lemma, lemma_mask, tags, tags_mask, gold, tf=True):
        ...
        for i in range(gold.size(1)):
            #print('train decode input', x.size(), hx.size())
        ...
        return hidden_states, F.leaky_relu(hcx), preds
    ...
    def decode_step(self, x, hx, cx, lemma, lemma_mask, tags, tags_mask):
        ...
        #print('x_seq_attn.size', x_seq_attn.size(), 'x.size', x.size())

        #print(hx.size(), x.size(), x_seq_attn.size(), x_tag_attn.size())
    ...
    def beam_decode(self, lemma, lemma_mask, tags, tags_mask):
        ...
        lemma_outputs, tags_outputs, ht, ct = self.encode_inputs(lemma, lemma_mask, tags, tags_mask)
        ...
        for idx in range(lemma.size(0)):
            ...
            #print('encoder sizes:', sent_lemma_outputs.size(), sent_tags_outputs.size())
            
            hx = start_hx[idx,:].unsqueeze(0)
            cx = start_cx[idx,:].unsqueeze(0)
            
            # Start with the start of the sentence token
            # set fist decoder input to <SOS>
            decoder_ids = torch.tensor([self.start_idx], device=device).long()
            decoder_input = self.char2emb(decoder_ids)
            ...
            #print('FIRST: decoded_t', decoder_ids, 'decoder_hidden', decoder_input.size())
            nodes = PriorityQueue()

            # start the queue
            k_score, k_node = -node.eval(), node
            nodes.put((k_score, k_node))
            #print('put1', k_score, k_node)
            qsize = 1
            ...
            while True:
                ...
                #decoder_preds = F.softmax(decoder_preds, 1)
...
class CosSimAttention(nn.Module):
    def __init__(self, in_dim, out_dim):
        ...
        #self.scale = nn.Linear(in_dim, out_dim, bias=False)
    ...
    def forward(self, k, xs, mask):
        ...
        #a_cos = F.log_softmax(a_cos, -1) * mask + EPS
