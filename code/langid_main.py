...
def save_model(model, e):
    torch.save(model.state_dict(), f'./models/sampl-{args.char_att}-lid_model_e={e}.pt')
...
def load_and_test_model():
    #data_pkl/data_test_release-20khall.pkl
    with open('data_pkl/data_test_release-10khall-special_symbs.pkl', 'rb') as f:
        _, _, test_data, lf, cf, tf, char_vocab, tag_vocab, lang_vocab = pickle.load(f)
    ...
    model = Inflector(char_vocab['<start>'], 
                      ...
                      len(ops_vocab.keys()))
    model.to(device)
    model.load_state_dict(torch.load(f'models/cos-bah-lid_model_e=29.pt'))
    #f'models/best_lid_model.pt'
    # Test
    test_iter = test_data_batcher(test_data, args.bs, char_vocab, tag_vocab, lang_vocab)
    #lid_data_batcher(dev_data, args.bs, char_vocab, tag_vocab, lang_vocab)
    test(0, model, char_vocab, lang_vocab, test_iter)
    #print('--- DEV ---')
    #print('EM beam search:', np.round(np.mean(em_test), 3))
    #print()
    
def data_sampler(dataset, languages, num_examples_per_lang=9000):
    # dataset = [(loss, example), ..., ()]
    uniform_lang_dataset = []
    all_idxs = [(i, x[1].lemmas[:,0]) for i, x in enumerate(dataset)]
    ...
    for lang in languages:
        ...
        while len(examples) < num_examples_per_lang:
            example_idx = random.choice(lang_idxs)
            uniform_lang_dataset.append(dataset[example_idx])
...
def main():
    ...
    proposed_path = f'runs/inflection_langid-opt={args.opt}-lr{args.lr}-wd{args.wd}-bs{args.bs}-d{args.hidden_dim}-orth={args.model_orthogonal_init}-curr={args.use_curriculum}-s={args.do_smoothing}-cs={args.initial_curriculum}-cos_att={args.char_att}'
    ...
    data_class = cl.CurriculumDataset(train_data, lang_vocab)
    #data_class = cl.CurriculumSamplerDataset(train_data, lang_vocab)
    ...
    for e in range(args.n_epochs):
        #train_data = data_class.data_sampler()
        train_iter = lid_data_batcher(train_data, args.bs, char_vocab, tag_vocab, lang_vocab, cl=args.use_curriculum)
        num_steps = len(train_iter)
        lr_scheduler = optim.lr_scheduler.CosineAnnealingLR(optimizer, T_max=num_steps, eta_min=args.min_lr)
        
        # Train
        model, curriculum_scores, tloss = train(e+1, model, char_vocab, train_iter, writer, optimizer, lang_chars, lr_scheduler)
        print('epoch', e, 'done')
        # Sort train examples by loss
        train_data = data_class.easyfirst_curriculum(curriculum_scores)
        #data_class.update_loss(curriculum_scores, idxs)
        ...
        if (e+1)%5 == 0:
            ...
            # Validate
            dev_outputs, vloss = validate(e+1, model, char_vocab, dev_iter, writer, lf, lang_vocab)
            loss_progression.append((tloss, vloss))
        
    test_iter = test_data_batcher(test_data, args.bs, char_vocab, tag_vocab, lang_vocab)
    test(0, model, char_vocab, lang_vocab, test_iter)
    
    with open('losses.csv', '+w') as f:
        for i, (t, v) in enumerate(loss_progression):
            f.write('\t'.join([str(i), str(t), str(v)])+'\n')
...
def train(e, model, char_vocab, data_iter, writer, optimizer, lang_chars, lr_scheduler=None, warmup_scheduler=None):
    ...
    for ex_i, example in enumerate(data_iter):
        ...
        decode_pred, opsy_pred, opsx_pred = model(example.lemmas,
                                                  lemma_mask,
                                                  example.tags,
                                                  tag_mask,
                                                  example.inflections,
...
def validate(e, model, char_vocab, data_iter, writer, dev_languages, lang_vocab):
    ...
    for i, example in enumerate(data_iter):
        ...
        with torch.no_grad():
            decode_pred, _, _ = model(example.lemmas,
                                                 lemma_mask,
                                                 example.tags,
                                                 tag_mask,
                                                 example.inflections,
    ...
    for k, v in sorted(outputs.items(), key=lambda x: x[0]):
        ...
        if k in ['ame', 'itl', 'ail', 'kod', 'gup', 'sjo', 'ckt', 'vro', 'bra', 'lud', 'evn', 'mag', 'see', 'syc', 'spa', 'deu']:
            print(k, em, np.round(np.mean(v['string_acc']), 3))
    ...
    print('DEV', e,
          ...
          np.round(np.mean(totals), 4))
    
    #print()
    #format_outputs(outputs)
    return outputs, np.mean(d_loss_history)
