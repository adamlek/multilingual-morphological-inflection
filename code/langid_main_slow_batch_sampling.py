...
def load_and_test_model():
    ...
    #dev_iter = test_data_batcher(dev_data, args.bs, char_vocab, tag_vocab, lang_vocab)
    #test(0, model, char_vocab, lang_vocab, dev_iter, gold, 'dev-with-test-format')
    
    #gold = [x[1] for x in dev_data]
    #dev_data = [dev_to_test_format(x) for x in dev_data]

    #test_iter = test_data_batcher(test_data, args.bs, char_vocab, tag_vocab, lang_vocab)
    #test(0, model, char_vocab, lang_vocab, test_iter, False, 'test')
...
def main():
    ...
    proposed_path = f'runs/inflection_langid-opt={args.opt}-lr{args.lr}-wd{args.wd}-bs{args.bs}-d{args.hidden_dim}-orth={args.model_orthogonal_init}-cs={args.initial_curriculum}-cos_att={args.char_att}_{args.model_orthogonal_init}_modules:{args.curriculum_learning}_{args.label_smoothing}_{args.multitask_learning}_{args.scheduled_sampling}'
...
def trainer_f(total_steps, model, curriculum, char_vocab, writer, optimizer, lang_chars, lang_vocab, lf, lr_scheduler, dev_iter):
    ...
    for ex_i in range(total_steps):
        ...
        if args.label_smoothing:
            ...
            decode_loss = _loss(decode_pred, example.inflections[:,1:], F.cross_entropy, model.c_pad, batch_weights)
...
def test(e, model, char_vocab, lang_vocab, data_iter, gold, fname):    
    itos = {v:k for k, v in char_vocab.items()}   
    lang_vocab_ = {i:x for x, i in lang_vocab.items()}
    
    test_out = open(f'examples/{fname}.txt', '+w')
    model.eval()
...
def _loss2(pred, gold, loss_fn, weights, ignore_idx=False):
    return (loss_fn(pred.view(-1,pred.size(-1)), 
                    gold.contiguous().view(-1),  
                    reduction='none')*torch.tensor(weights, device=device)).sum()/pred.size(0)

def _loss(pred, gold, loss_fn, ignore_idx=False, weights=False):
    if ignore_idx:
        return loss_fn(pred.view(-1,pred.size(-1)), gold.contiguous().view(-1), ignore_index=ignore_idx)*np.mean(weights)
    else:
