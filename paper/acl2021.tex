...
statistical and rule-based methods.\jp{citation(s) needed}. In rule-based methods rules for
...
This is a challenging problem.\jp{for the main reason, for several reasons ?} For many  languages, resources 
are scarce, so a multilingual system must be able to model inflectional
...
$100310$ (Turkish) presenting a unique challenge as a multilingual
systems.
...
produce a new inflected form by replacing the parts of an existing inflected form
...
\citet{DBLP:conf/emnlp/AnastasopoulosN19}, we make the following changes. When replacing a sequence of
...
removing up to $2$ characters.\jp{I don't understand this} Additionally, we don't consider
...
Cosim by \jp{TODO: citation}).  However, we found that using Cosim only
causes attention to be too focused on a single character, and mostly
ignores contextual cues relevant for the generation.
...
\textbf{lemma-additions}. To illustrate, the levenshtein operations for the \jp{source language?} word pair
...
We use the easy-first curriculum strategy\jp{[REF]} to sort the data after
...
For the first epoch, when we dont have any loss refer to for sorting, we sort
...
of the first epoch, which persists throughout the learning process.\jp{Curriculum learning in general or the specific thing for the 1st epoch?}

\subsection{Scheduled Sampling}

Commonly, when training an encoder-decoder RNN model, the input at
time-step $t$, is not the output from the decoder at $t-1$, but rather the gold data.
...
we decrease \(\rho\) by 4\%. For each character, we take a sample from the
