...
We employ a encoder-decoder architecture with attention. First\jp{where is Second?} we use
a LSTM to obtain a contextual representation for each word in the
lemma. We encode the tags using a self-attention module\jp{what is that? be more precise}, as the order
of the tags does not matter.\jp{tags were never defined. I suppose a description of the task should come before, defining: lemma, tags, inflected form, etc.}

The encoder has three parts, a LSTM generator\jp{rnn?} and two attention
...
However, only using cosim\jp{not introduced} attention was shown to have pitfalls\jp{citation needed}, as it
mainly focused on the most similar\jp{similar to what?} characters in the lemma it ignored
...
$add$ additive attention and $k$ the key:\jp{\(a^{add}\) has conflicting definitions}
\jp{Don't use text for functions. mathrm is similar but better for typography.}
\begin{align*}
...
We break down the task of applying levenshtein-distance\jp{never introduced. Why do you want to do this?} operations
...
fewest addition-operations, least-grammatical-features, and random.\jp{by how much?}

\subsection{Scheduled Sampling}

It has been shown that models trained with teacher-forcing\jp{what's that?} may suffer
...
We use KL-divergence loss for the characters\jp{Why not cross-entropy?} and cross-entropy loss
...
set may not be accounted for.\jp{Isn't that precisely what label smoothing is for?}

\paragraph{Learning rate decay with a Curriculum} The outputs\jp{of what? in training isn't everything input?} will be
sorted by the difficulty in the previous epoch. To further follow the
easy-first learning idea we employ decaying learning rate.\jp{Imo this is orthogonal?} This has
